...
//    Description: The goal of this class it calculate the drowsiness of the operator using a camera.
//
...
//NOTE: Change this to 1 for use with Raspberry Pi
#define USE_RP_CAM 0
...
/*
neural network not in use NOTE: NEED TO INCLUDE THE MLP AND THE NEURAL NETWORK PACKAGE FROM DLIB.
void neural_train(mlp::kernel_1a_c &net, std::vector<full_object_detection> facialFeatures, cv::Mat euler_angle, double expected_result) {
	typedef matrix<double, 139, 1> sample_type;
	sample_type samples;
	for (int i = 0; i < 136; i = i + 2) {
		samples(i) = facialFeatures[0].part(i).x();
		samples(i + 1) = facialFeatures[0].part(i).y();
	}
	samples(136) = euler_angle.at<double>(0);
	samples(137) = euler_angle.at<double>(1);
	samples(138) = euler_angle.at<double>(2);
	int result = 0;
	if (expected_result > 0) {
		result = 1;
	}
	net.train(samples, result);
	cout << "result: " + std::to_string(net(samples)) << endl;
